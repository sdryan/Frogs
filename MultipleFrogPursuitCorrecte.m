...
%     clear
    ...
    for j = 1:N
        xf(1,j) = 4.2;%rand(1) * BoxL;%BoxL/2.0; 
        yf(1,j) = 6.3;%rand(1) * BoxW;%BoxW/2.0;
        thetad = rand * 2 * pi;
        dxf(j) = cos(thetad);
        dyf(j) = sin(thetad);
        theta2 = rand * 2 * pi; 
    ...
    for k = 1:M
        xr(1,k) = 8.8;%rand(1) * BoxL; 
        yr(1,k) = 1.6;%rand(1) * BoxW;
        theta1 = rand * 2 * pi; 
    ...
    for i = 1:T 
       ...
       [Q,I] = min(distance); %Q(j) = min distance for frog j
        ...
        for k = 1:M  %trying to add all the twitching terms before computing new velocity. May or may not be correct.
            if (roach_alive(k) == 1)
        twitchingx(k) = 0;
        twitchingy(k) = 0;
        ...
        for k = 1:M
            if roach_alive(k) == 1
        vrx(i+1,k) = vrx(i,k) + dt*(alphar*vrx(i,k)*(vr0*vr0-(vrx(i,k)^2+vry(i,k)^2))) + Dr*normrnd(0,sqrt(dt)) + twitchingx(k);%Dr*normrnd(0,sqrt(dt))+dt*(1-max(phi(j)))*Dtw*(xf(i,j)-xr(i,k))/sqrt((xf(i,j)-xr(i,k))^2+(yf(i)-yr(i,k))^2);
        vry(i+1,k) = vry(i,k) + dt*(alphar*vry(i,k)*(vr0*vr0-(vrx(i,k)^2+vry(i,k)^2))) + Dr*normrnd(0,sqrt(dt)) + twitchingy(k);%Dr*normrnd(0,sqrt(dt))+dt*(1-max(phi(j)))*Dtw*(yf(i,j)-yr(i,k))/sqrt((xf(i,j)-xr(i,k))^2+(yf(i)-yr(i,k))^2);
        %-Update bug position-
    ...
    %% -----plot motion-----
        ...
        for j = 1:N
        plot(xf(i+1,j),yf(i+1,j),'go','MarkerSize',75,"MarkerFaceColor","g") %could change frog size or color
        end
        axis([0 BoxL 0 BoxW])
        ...
        for j = 1:N
            if (Q(j)<tong_rad)%Also need to add indicator for which frog caught the bug.
...
writematrix(time,'CalibrationCheck2.11.24.csv');
% add delay in frog sensing bug?